    ...
    <>
      <style>{`
        ...
        *,*::before,*::after { box-sizing:border-box; margin:0; padding:0; }
        ...
        .solde-banner { display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:1rem; background:rgba(124,58,237,.07); border:1px solid rgba(124,58,237,.22); border-radius:16px; padding:1.1rem 1.5rem; margin-bottom:2rem; position:relative; overflow:hidden; }
        ...
        .solde-cta:hover { background:#6d28d9; }
        .why-grid { display:grid; grid-template-columns:repeat(3,1fr); gap:.75rem; margin-bottom:2.5rem; }
        .why-card { background:rgba(255,255,255,.02); border:1px solid rgba(255,255,255,.06); border-radius:16px; padding:1rem; }
        .why-icon { font-size:1.25rem; margin-bottom:6px; }
        .why-title { font-weight:700; font-size:.82rem; color:#fff; margin-bottom:4px; }
        ...
        .annonce-card { background:rgba(255,255,255,.03); border:1px solid rgba(255,255,255,.07); border-radius:16px; padding:1.1rem 1.25rem; display:flex; align-items:center; gap:12px; cursor:pointer; transition:all .2s; margin-bottom:.625rem; animation:fadeIn .3s ease; position:relative; overflow:hidden; }
        ...
        .annonce-card.boosted-active { border-color:rgba(251,191,36,.3); background:rgba(251,191,36,.04); cursor:default; }
        .annonce-checkbox { width:20px; height:20px; border-radius:6px; flex-shrink:0; border:1.5px solid rgba(255,255,255,.2); transition:all .18s; display:flex; align-items:center; justify-content:center; }
        ...
        .annonce-code { font-family:'Courier New',monospace; font-size:.78rem; color:rgba(255,255,255,.35); letter-spacing:.06em; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; max-width:260px; }
        ...
        .tag-boosted { background:rgba(251,191,36,.12); border:1px solid rgba(251,191,36,.3); color:#fbbf24; }
        ...
        .summary-card { position:sticky; bottom:24px; background:rgba(10,10,15,.92); backdrop-filter:blur(16px); border:1px solid rgba(124,58,237,.3); border-radius:18px; padding:1.1rem 1.5rem; display:flex; align-items:center; justify-content:space-between; gap:1rem; flex-wrap:wrap; box-shadow:0 8px 32px rgba(124,58,237,.2); margin-top:1.5rem; }
        ...
        .boost-btn { display:inline-flex; align-items:center; gap:7px; background:#7c3aed; color:#fff; border:none; border-radius:12px; padding:.65rem 1.5rem; font-size:.9rem; font-weight:700; cursor:pointer; transition:all .2s; font-family:'DM Sans',sans-serif; white-space:nowrap; }
        ...
        @media(max-width:600px) { .page{padding:2rem 1rem 5rem} .why-grid{grid-template-columns:1fr} .summary-card{flex-direction:column;align-items:stretch} .boost-btn{justify-content:center} }
      `}</style>
      ...
      <main className="page">
        ...
        <p className="section-title">
          Mes annonces
          <span style={{ fontSize:".72rem", color:"rgba(255,255,255,.3)", fontFamily:"'DM Sans',sans-serif", fontWeight:400 }}>
          ...
          >
            {!a.boosted && (
              <div className="annonce-checkbox">
                {selected.includes(a.id) && <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="3"><polyline points="20 6 9 17 4 12"/></svg>}
              </div>
            )}
        ...
        {!loading && annonces.some(a => !a.boosted) && (
          <div className="summary-card">
            ...
            <button className="boost-btn" disabled={!canBoost} onClick={handleBoost}>
              ...
              {boosting ? "Activation..." : "Activer le boost"}
            </button>
          </div>
        )}
